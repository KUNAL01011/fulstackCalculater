...
function octalToDecimal(octalString) {
    let decimal = 0;
    let bits = octalString.split('.');
    let integerPart = bits[0];
    let fractionalPart = bits[1] || '0';
    for (let i = 0; i < integerPart.length; i++) {
        decimal += parseInt(integerPart[i]) * Math.pow(8, integerPart.length - 1 - i);
    }
    for (let i = 0; i < fractionalPart.length; i++) {
        decimal += parseInt(fractionalPart[i]) * Math.pow(8, -(i + 1));
    }
    return decimal;
}
